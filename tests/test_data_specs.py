...
from climate_index_collection.data_specs import (
    get_spacial_dimension_specs,
    latitude_longitude_specs,
    sel_latitude_longitude_slice,
)
...
def compare_dict_items(d1, d2):
    ...
    for key in d1.keys():
        ...
        d2_item = d2[key]
        try:
            comparison.append(np.array_equal(d1_item, d2_item, equal_nan=True))
...
@pytest.mark.parametrize(
    "data, lat_bounds, lon_bounds, lat_slice_should, lon_slice_should",
    [
...
def test_sel_latitude_longitude_slice(
    data, lat_bounds, lon_bounds, lat_slice_should, lon_slice_should
):
    """Check if the dictonary contains all keys and if the values are correct."""
    LatSlice, LonSlice = sel_latitude_longitude_slice(
        dobj=data, LatBounds=lat_bounds, LonBounds=lon_bounds
    )
    # check if resulting slices are correct
    assert all((LatSlice == lat_slice_should, LonSlice == lon_slice_should))
